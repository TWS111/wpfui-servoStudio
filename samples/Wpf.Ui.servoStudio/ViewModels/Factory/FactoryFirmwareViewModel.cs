...
public partial class FactoryFirmwareViewModel : ViewModel
{
    ...
    private async Task RefreshNetworkStatusAsync()
    {
        ...
        try
        {
            ...
            // SMB 探测整体超时兜底（IsCompanyNetworkAvailable 内部首次冷启动可能 1s+3s 重试，
            // 这里再加一层 10s 整体超时覆盖 LoadAll 阶段，避免极端情况下 UI 长时间停在加载蒙版）。
            Task winner = await Task.WhenAny(probeTask, Task.Delay(TimeSpan.FromSeconds(10)));
    ...
    private void OnBrowseFile()
    {
        ...
        {
            Title = "选择要上传的固件文件",
            Filter = "固件 / ESI XML (*.bin;*.hex;*.efw;*.fw;*.xml)|*.bin;*.hex;*.efw;*.fw;*.xml|固件文件 (*.bin;*.hex;*.efw;*.fw)|*.bin;*.hex;*.efw;*.fw|ESI XML (*.xml)|*.xml|所有文件 (*.*)|*.*",
            Multiselect = false,
        };
        ...
        // ESI XML 上传预设：状态强制为“发布”，隐式标记为外部已签名（XML 本身即为可封装产物，
        // 不走 pack_ota.py；标记后可使 UploadAsync 保留“发布”状态而不被强制回“未签名”）。
        if (string.Equals(Path.GetExtension(dialog.FileName), ".xml", StringComparison.OrdinalIgnoreCase))
        {
            SelectedStatus = FirmwareRepositoryService.PublishedStatus;
            SignFirstBeforeUpload = false;
            MarkExternallySignedOnUpload = true;
        }
    ...
    private async Task OnBurn(FirmwareEntry? entry)
    {
        ...
        bool isBin = string.Equals(ext, ".bin", StringComparison.OrdinalIgnoreCase);
        bool isXml = string.Equals(ext, ".xml", StringComparison.OrdinalIgnoreCase);
        ...
        if (!isBin && !isXml)
        {
            ...
            {
                Title = "不支持的文件类型",
                Content = $"当前仅支持 .bin（FoE 固件烧录）与 .xml（EtherCAT 从站XML烧录）。\r\n该条目实际文件为：\r\n{System.IO.Path.GetFileName(filePath)}",
                CloseButtonText = "确定",
                CloseButtonAppearance = Wpf.Ui.Controls.ControlAppearance.Primary,
                Owner = Application.Current?.MainWindow,
            }.ShowDialogAsync();
        ...
        // 通过 DI 取出 FirmwareProgramViewModel 单例并预填文件信息。
        ...
        try
        {
            ...
            if (isBin)
            {
                // FoE 固件烧录（Tab 0）：厂家库内的可烧录 .bin 均为已签名状态，
                // 自动勾选"已签名固件 (跳过 OTA 打包)"，并把目标文件名置为约定的 "app"。
                ...
                programVm.FoeTargetFileName = "app";
                programVm.SelectedTabIndex = 0;
                programVm.StatusText = $"已从厂家库载入: {fi.Name} (FoE 目标: app)";
            }
            else
            {
                // EtherCAT 从站XML烧录（Tab 1）：直接置入 ESI 文件相关属性。
                programVm.EsiFilePath = filePath;
                programVm.EsiFileName = fi.Name;
                programVm.EsiFileSize = fi.Length;
                programVm.IsEsiFileSelected = true;
                programVm.SelectedTabIndex = 1;
                programVm.EsiStatusText = $"已从厂家库载入: {fi.Name}";
            }
        }
        catch (Exception ex)
        {
