...
public partial class MotorViewModel(DeviceAddViewModel deviceAddViewModel) : ViewModel
{
    ...
    private async Task OnReadAllParameters()
    {
        ...
        try
        {
            ...
            AppData.AppLogViewModel.Log(AppLogLevel.Info, AppLogCategory.SDO, "电机参数读取完成",
                $"成功 {successCount}, 失败 {failCount}");
        }
        catch (Exception ex)
        {
            StatusText = $"读取异常: {ex.Message}";
            AppData.AppLogViewModel.Log(AppLogLevel.Error, AppLogCategory.SDO, "电机参数读取失败", ex.Message);
        }
        finally
        {
            IsBusy = false;
    ...
    private async Task OnWriteModifiedParameters()
    {
        ...
        try
        {
            ...
            AppData.AppLogViewModel.Log(AppLogLevel.Info, AppLogCategory.SDO, "电机参数写入完成",
                $"成功 {successCount}, 失败 {failCount}");
        }
        catch (Exception ex)
        {
            StatusText = $"写入异常: {ex.Message}";
            AppData.AppLogViewModel.Log(AppLogLevel.Error, AppLogCategory.SDO, "电机参数写入失败", ex.Message);
        }
        finally
        {
            IsBusy = false;
